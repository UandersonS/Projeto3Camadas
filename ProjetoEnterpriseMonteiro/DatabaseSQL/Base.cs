    ...
    public abstract class Base : IPessoa
    {
        ...
        public virtual void Gravar()
        {
            ...
            using (SqlConnection connection = new SqlConnection(
               connectionString)) //pegando String de conexão no aquivo de configuração na camada de interface
            {
                string queryString = "insert into " + this.GetType().Name + "s (nome, telefone, cpf)values('" + this.Nome + "', '" + this.Telefone + "', '" + this.CPF + "');";
                SqlCommand command = new SqlCommand(queryString, connection);
                //+ this.GetType().Name + é responsável por pegar o nome da classe "USUARIOS"
